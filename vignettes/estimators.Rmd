...
`"hr"`    | Hazard ratio from Cox proportional hazards regression. | `list(robust = TRUE)` for robust (sandwich standard) errors; `list(weights = "weight_variable", robust = TRUE)` for sample (e.g., inverse-probability) weights and robust errors. Use `"+ cluster(id_variable)"` in `confounders` to obtain clustering.
