...
* Clerical errors in variable names and arguments -- no magic here except double-checking the code.
* Missing data -- see below: [How do I handle missing data?](#how-do-i-handle-missing-data).
* Discrepancy between estimator (`type`) and the data -- for example, when trying to calculate `type = "mean"` on a categorical (factor) variable.
* Models that fail to converge -- for example, trying to estimate a risk ratio with 0 outcomes in the reference category, or adjusting for 20 covariates in a Cox model and 5 events overall. (Sometimes such attempts will "just" return warning messages -- still rethink the modeling strategy.)

To identify where an error is coming from, start simple. Comment out all but one line of the `design`, putting `#` at the beginning of the line. Start with a line that gives basic descriptive data, such as `type = "total"`, `type = "outcomes"` or `type = "events/time"`. Success? Then add more lines with descriptive estimators, one by one. At the end, add lines that fit models, such as `type = "hr"`.
...
The `design` could even be written out in an external dataset that can be loaded
with `readr::read_csv()` (for CSV files) or `readxl::read_excel()` (for Excel sheets).
...
* Missing values in **outcome**: By default, descriptive statistics will be missing, and regression models will use the non-missing observations. To change exclude observations with missing outcome altogether, add `na_rm = TRUE` to (specific rows of) the `design`.
