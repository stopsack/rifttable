...
* Note for `table1_design()`: Missing values (`NA`) for categorical variables in rows of a Table 1 are displayed by counts but without percentages. It is typically more reasonable to assume that observations with missing values fall into one of the existing categories than to assume that they fall into an entirely different category that otherwise does not exist. For example, in a study, `"Asia"` (*n* = 500; 50%) and `"Other continents"` (*n* = 500; 50%) may be possible values for `continent`. Study participants whose `continent` is `NA` (*n* = 200) likely belong into one of those two categories and not into another, separate category that did not apply to any of the participants with non-missing data ("extraterrestrial"?). Thus, a category for missing values should generally not be included in the denominator when calculating percentages, and rifttable operates accordingly. In unusual cases in which missingness is a real, informative category in the target population, not just the data at hand, use `forcats::fct_na_value_to_level()` to convert `NA` into a factor level before passing the `data` to `rifttable()`.
...
Our simple example dataset just has one exposure variable. For demonstration,
