...
The `breastcancer` dataset of a binary outcome is included in the package:
```{r data1}
data(breastcancer)
...
The `cancer` dataset from survival package is used in some other examples:
```{r data2}
...
* `gt::gt()`: supports more advanced formatting. If using `rifttable::rt_gt()` as a wrapper for `gt::gt()`, indentations from the `label` of the table `design` are properly passed along, and tables are shown properly if the markdown is knitted to `github_document` (just a `.md`, not an `.html` or other formats).
...
# Example 7: Custom functions 

This example also adds an overall, unstratified estimate.
...
my_custom_fun1 <- function(data, ...) {
  data |>
    group_by(.exposure) |>
    # Directly referencing the outcome variable 'status'--not portable
    summarize(res = paste("mean =", mean(status)))
}

# Define second, improved function
my_custom_fun2 <- function(data, outcome, ...) {
  # 'outcome', like all columns of the 'design', is one argument
  # that is passed along to custom functions
  data |>
    # The exposure variable is always renamed in '.exposure':
    group_by(.exposure) |>
    # This improved function will work in data sets regardless of how
    # what the name of the 'outcome' variable is
    select(
      .exposure, 
      variable = {{ outcome }}) |>
    summarize(
