...
table_regress <- function(
  ...
  if(stringr::str_detect(
    string = estimand, pattern = "_joint") &
  ...
  if(stringr::str_detect(
    ...
    if(is.na(tau))
      stop(
        paste0(
          "The supplied quantile is not a valid numeric value: '",
          estimand, "'."))
    if(stringr::str_detect(
      string = estimand,
      pattern = "_joint"))
  ...
  if(stringr::str_detect(
    ...
    if(!(approach %in% possible_approaches)) {
      warning(
        paste0(
          "Fitting a risks model via approach '",
          approach,
  ...
  if(stringr::str_detect(
    ...
    estimand_joint <- stringr::str_extract(
      string = estimand,
      pattern = "_joint|")
    estimand <- stringr::str_extract(
      string = estimand,
      pattern = "cumincdiff|survdiff")
    surv_cuminc <- dplyr::if_else(
      estimand == "survdiff",
      true = "survival",
      false = "cuminc")
    ...
    if(stringr::str_length(string = timepoint) > 0) {
      if(is.na(as.numeric(timepoint)))
        stop(paste0(
          "The supplied time horizon is not ",
  ...
  fit <- switch(
    ...
    cumincdiff_joint = {
      fit %>%
        dplyr::mutate(term = paste0(".exposure", .data$term))
    },
    irrrob =,
    irrrob_joint = {
