  ...
  if(stringr::str_detect(
    ...
    if(stringr::str_length(string = timepoint) > 0) {
      if(is.na(as.numeric(timepoint)))
        stop(
          paste0(
            "The supplied time for cumulative incidence is not ",
        ...
        if(type == "medsurv (ci)") {
          ...
          paste0(
            trimws(
              format(
                round(
                  quantile(fit, probs = 0.5)$quantile[[1]],
                  digits = digits),
                nsmall = digits)),
            " (",
            trimws(
              format(
                round(
                  quantile(fit, probs = 0.5)$lower[[1]],
                  digits = digits),
                nsmall = digits)),
            to,
            trimws(
              format(
                round(
                  quantile(fit, probs = 0.5)$upper[[1]],
                  digits = digits),
                nsmall = digits)), ")")
        } else { "" },
      type == "medfu" ~
        ...
        if(type == "medfu (iqr)") {
          ...
          paste0(
            ...
            trimws(
              format(
                round(
                  quantile(fit, probs = 0.25)$quantile[[1]],
                  digits = digits),
                nsmall = digits)),
            to,
            trimws(
              format(
                round(
                  quantile(fit, probs = 0.75)$quantile[[1]],
                  digits = digits),
                nsmall = digits)),
            ")")
        } else { "" },
