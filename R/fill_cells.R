...
fill_cells <- function(
  ...
  if(!is.na(digits)) {
    if(!is.numeric(digits))
      stop(
        paste0(
          "'digits' value for rounding, if given, must be numeric. '",
          digits,
          "'is not numeric."))
  ...
  if(stringr::str_detect(
    ...
    if(stringr::str_detect(
      ...
      if(is.null(custom_fn))
        stop(
          paste0(
            "Custom function requested via type = '",
            type,
            "', but ",
      ...
      if(is.na(type)) {
        if(is.list(custom_fn))
          stop(
            paste0(
              "More than one custom function provided via",
              "table2(..., custom = <list>), but 'type' ('",
              type,
        ...
        if(!is.list(custom_fn) &
          ...
          stop(
            paste(
              "One or fewer custom functions provided via",
              "table2(..., custom = <list>), but 'type' ('",
              type,
